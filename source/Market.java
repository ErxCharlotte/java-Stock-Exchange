...
public class Market{
    ...
    public List<Trade> placeSellOrder(Order order){
        ...
        double traderAmount = trader.inventory.get(product);
        if (traderAmount < order.getAmount()){
            return null;
        }
        sellBook.add(order);
        double nowNum = (order.getTrader()).inventory.get(product) - order.getAmount();

        //Determine whether the book is empty.
        if (buyBook.size() == 0){
            order.getTrader().inventory.replace(product, nowNum);
        ...
        for (int u = 0; u < sameProductInBuyBook.size(); u ++){
            ...
            dealWithSellOrders(order, sameProductInBuyBook.get(u), completedSellTrades);
        }
